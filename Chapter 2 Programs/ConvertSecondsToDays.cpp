...
int main ()
{
	int time, d_t, d_r, hr_t, r_hr, m_t, r_m, s_t;
	const int d= 86400, hr = 3600, min = 60, sec = 1;
	...
	(d_t = time/d);
	(d_r = time%d);
	cout << "Your time in days is " << d_t << "." << endl
		 << endl;
	...
	cout << "can be written as: " << d_t << " days," << hr_t << " hours, " << m_t << " minutes, and " << s_t << " seconds. \n";
	cout << endl;
